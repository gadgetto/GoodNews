...
class GoodNewsSubscriptionValidator {
    ...
    /**
     * The constructor for the GoodNewsSubscriptionValidator class.
     *
     ...
     * @return GoodNewsSubscriptionValidator
     */
    function __construct(GoodNewsSubscription &$goodnewssubscription, array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators'             => !empty($this->goodnewssubscription->config['customValidators']) ? explode(',', $this->goodnewssubscription->config['customValidators']) : array(),
        ), $config);
    }
     ...
     *
     * @access public
     * @param GoodNewsSubscriptionDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(GoodNewsSubscriptionDictionary $dictionary, $validationFields = '') {
    ...
    public function validate($key, $value, $type = '') {
        ...
        /* only allow specified validators to prevent brute force execution of unwanted snippets */
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->goodnewssubscription->config, array(
                    'key'       => $key,
                    'value'     => $value,
                    'param'     => $param,
                    'type'      => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR, '[GoodNews] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO, '[GoodNews] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key, $value) {
        $errTpl = $this->modx->getOption('errTpl', $this->goodnewssubscription->config, '<span class="error">[[+error]]</span>');
    ...
    public function blank($key, $value) {
        return empty($value) ? true : $this->_getErrorMessage($key, 'vTextBlank', 'goodnews.validator_field_not_empty', array(
            'field' => $key,
            'value' => $value,
        ));
    }
    ...
    public function email($key, $value) {
        ...
        if (!$condition) {
            return $this->_getErrorMessage($key,'vTextEmailInvalid', 'goodnews.validator_email_invalid', array(
                'field' => $key,
                'value' => $value,
            ));
        }
        ...
        if (!$condition) {
            ...
            if (sizeof($domain_array) < 2) {
                return $this->_getErrorMessage($key, 'vTextEmailInvalidDomain', 'goodnews.validator_email_invalid_domain', array(
                    'field' => $key,
                    'value' => $value,
                ));
            }
    ...
    public function minLength($key, $value, $param = 0) {
        ...
        if ($v < $param) {
            return $this->_getErrorMessage($key, 'vTextMinLength', 'goodnews.validator_min_length', array(
                'length' => $param,
                'field'  => $key,
                'value'  => $value,
            ));
    ...
    public function maxLength($key, $value, $param = 999) {
        $v = $this->config['use_multibyte'] ? mb_strlen($value,$this->config['encoding']) : strlen($value);
        if ($v > $param) {
            return $this->_getErrorMessage($key, 'vTextMaxLength', 'goodnews.validator_max_length', array(
                'length' => $param,
                'field'  => $key,
                'value'  => $value,
            ));
    ...
    protected function _getErrorMessage($field, $parameter, $lexiconKey, array $properties = array()) {
        if (!empty($this->goodnewssubscription->config[$field.'.'.$parameter])) {
        
            $message = $this->goodnewssubscription->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey, $message);
            $this->modx->lexicon($lexiconKey, $properties);
            
        } else if (!empty($this->goodnewssubscription->config[$parameter])) {
        
            $message = $this->goodnewssubscription->config[$parameter];
            $this->modx->lexicon->set($lexiconKey, $message);
            $this->modx->lexicon($lexiconKey, $properties);
